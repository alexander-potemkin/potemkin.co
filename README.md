...
## Как сделать пост
Посты должны обязательно находится в папке `_posts`. Чтобы создать пост, необходимо добавить в папку `_posts` файл с названием `YEAR-MONTH-DAY-TITLE.md`. То есть примером валидного имени поста будет `2011-12-31-new-years-eve-is-awesome.md`. `TITLE`, который вы пишете в названии файла, не влияет на название поста на самом сайте. На сайте отображается значение `title:` из шапки поста.

В начале каждого поста должна быть шапка, которая выглядит следующим образом (три знака тире обязательны в начале и в конце шапки):
```
---
layout: post
title:  a post with something
description: march & april, looking forward to summer
tags: formatting links
categories: sample-posts
---
```
1. title, description, categories, tags (и дата из названия) будут показываться в обложке поста на индексной странице (вкладка writings). Во вкладке с навигацией toc (там показываются посты по категориям), будут видны только дата и title поста.
2. Чтобы добавлять пост в закрепленные, нужно добавить в шапку строку `featured: true`
3. [Здесь](https://alshedivat.github.io/al-folio/blog/) можно посмотреть на то, как выглядят шаблоны постов. (можно зайти в папку с постами [репозитория](https://github.com/alshedivat/al-folio/tree/master/_posts) al-folio и найти по названию markdown понравившегося шаблона)
4. Примеры полезных шаблонов: [блоки кода и текста, покрашенные в разные цвета](https://alshedivat.github.io/al-folio/blog/2023/custom-blockquotes/) (можно использовать для каких-то инструкций), [пост с навигацией](https://alshedivat.github.io/al-folio/blog/2023/sidebar-table-of-contents/), [пост с картинками](https://alshedivat.github.io/al-folio/blog/2015/images/), [пост с математикой](https://alshedivat.github.io/al-folio/blog/2015/math/) [Здесь](https://jekyllrb.com/docs/posts/#including-images-and-resources) описан самый простой способ вставки в посты картинок и pdf (аналогичен вставке ссылок в markdown)
##### Дополнительно:
- Можно также прописывать в шапке строку с датой (например, чтобы посты, добавленные в один день, показывались на сайте в определенном порядке) следующим образом: `date: 2015-03-15 16:40:16`. Этой датой вы перекрываете дату, которую писали в названии файла.
- можно включить функцию комментирования постов с регистрацией через github (сейчас она не активирована для сайта potemkin.co) через добавление в шапку строки `giscus_comments: true`. В этом, как и в других подобных случаях, можно просто поставить значение `false`, если вы не хотите этим пользоваться, чтобы не удалять из шаблона эту строку
- также можно добавить строку `related_posts: true` чтобы в конце поста выдавались последние посты со схожими хэштэгами. В файле `_config.yml` выставляется значение `max_related`, которое ограничивает максимальное количество выдаваемых постов
- [здесь](https://github.com/alexander-potemkin/potemkin.co#layouts) можно дополнительно прочитать про такую разметку как distill

## Категории и тэги
Чтобы хэштэги и категории высвечивались шапке с навигацией на индексной странице, нужно добавить их в файле `config.yml` в массив `display_tags` и `display_categories`
Например так: `display_categories: ['category', 'movies', 'horrors']`.
- основное отличие категорий от тэгов в том, что путь к файлу может определять категорию. Пример: вы хотите сделать пост с отзывом на фильм, тогда можно сделать в корневой папке папку `movies`, в ней папку `horrors`, внутри папку `_posts`, и уже внутри нее файл markdown с отзывов на фильм, тогда к вашим категориям поста автоматически добавятся категории horrors и movies. (не забудьте добавить эти категории в `_config.yml`, если хотите, чтобы по этим категориям можно было навигироваться с индексной страницы; во вкладке `toc` эти категории добавятся автоматически)
- То есть идея в том, что категории можно в каждом посте вообще не прописывать руками, а просто сделать иерархическую структуру с файлами по схожим категориям (правда тогда во вкладке `toc` будет отдельно список постов с категорией `horrors` и `movies`, т.е. некоторые посты будут дублироваться)
- как уже было упомянуто, по хэштэгам в конце поста будут выдаваться посты со схожими хэштэгами, если в шапке поста вставить строку `related_posts: true`
Оригинальный текст про тэги и категории (а также про создание постов) можно прочитать в [документации](https://jekyllrb.com/docs/posts/#tags-and-categories)
## Отступы
Ширину контента можно менять в файле `_config.yml` - 54 строка, значение `max_width`
Например `max-width: 800px`или  `max-width: 80%`
## Акцентный цвет 
- Акцентный цвет можно менять в файле `_sass/_themes.scss`, значение `--global-theme-color: #{$orange-color};` и 
  `--global-hover-color: #{$orange-color};` - соответственно `hover-color` - это цвет, в который окрашивается текст, когда мы подносим к нему курсор
- Аналогичные значение можно поменять для темной темы в блоке кода `html[data-theme='dark']` файла `_sass/_themes.scss`
- Переменные `orange-color` и подобные содержатся в файле `_sass/_variables.scss`, но можно задавать цвета в файле `_sass/_themes.scss` напрямую: например, как `rgba(0,0,0,.1)` или `#42b983`

## Почта
Можно добавить ссылку на свою почту (которая активируется при клике на иконку почты на вкладке `about`) в файле `_config.yml` в значении `email:` (9 строка)
