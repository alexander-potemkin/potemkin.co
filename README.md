...
## Как сделать пост
Посты должны обязательно находится в папке `_posts`. Чтобы создать пост, необходимо добавить в папку `_posts` файл с названием `YEAR-MONTH-DAY-TITLE.md`. То есть примером валидного имени поста будет `2011-12-31-new-years-eve-is-awesome.md`.
...
##### Дополнительно:
- Можно также прописывать в шапке строку с датой (например, чтобы посты, добавленные в один день, показывались на сайте в определенном порядке) следующим образом: `date:   2015-03-15 16:40:16`
